...
# Dropout 추가된 convnextv2_tiny 클래스 정의
class ConvNeXtV2WithDropout(nn.Module):
    def __init__(self, num_classes=7, dropout_p=0.3):
        super().__init__()
        self.model = create_model("convnextv2_tiny", pretrained=True)
        in_features = self.model.head.fc.in_features
        self.model.head.fc = nn.Sequential(
            nn.Linear(in_features, 512),
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Dropout(p=dropout_p),           # 첫 번째 Dropout
            nn.Linear(512, 256),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.Dropout(p=dropout_p),           # 두 번째 Dropout
...
val_loader = DataLoader(val_dataset, batch_size=CFG['BATCH_SIZE'], shuffle=True, num_workers=4, pin_memory=False, prefetch_factor=2)
...
if __name__ == '__main__':
    ...
    resume_training = False # <- 이어서 할지 여부

    # # 모델 
    model = ConvNeXtV2WithDropout(num_classes=7,dropout_p=0.3).to(device)

    # # <- 이걸로 할 경우 위에 모델 정의 할 때 pretrained=False로 해줘야함
    # model.load_state_dict(torch.load('./output/model_epoch_50.pth', map_location=device)) 
    ...
    model= train(model, optimizer, train_loader, val_loader, scheduler, device,  freeze_backbone=False, start_epoch=start_epoch, epochs=50,best_score=best_score,stage=2)

    print("추론 시작")
    preds = inference(model, test_loader, device)

    submit_tta = pd.read_csv('./sample_submission.csv')
    submit_tta['rock_type'] = preds
    submit_tta.to_csv('./baseline_submit_0526.csv', index=False)

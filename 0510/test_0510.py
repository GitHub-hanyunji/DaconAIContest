...
train_transform = A.Compose([
    PadSquare(value=(0, 0, 0)),  # 이미지 정사각형으로 패딩 (중앙 정렬)
    A.RandomResizedCrop(CFG['IMG_SIZE'], CFG['IMG_SIZE'], scale=(0.8, 1.0), p=0.5),  # 랜덤 크롭 + 리사이즈 → 다양한 위치 강조
    A.CLAHE(clip_limit=4.0, tile_grid_size=(8, 8), p=0.5),  # contrast-limited histogram equalization → 질감 강조
    A.Sharpen(alpha=(0.1, 0.3), lightness=(0.8, 1.0), p=0.3),  # 샤프닝 → 경계 및 패턴 강화
    A.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.05, p=0.3),  # 색상/채도/명도 다양화 → 광원 환경 대응
    A.GaussNoise(var_limit=(10.0, 50.0), p=0.3),  # 가우시안 노이즈 추가 → 질감 다양화 및 강건성 향상
    A.ShiftScaleRotate(shift_limit=0.02, scale_limit=0.1, rotate_limit=15, p=0.3),  # 이동/확대/회전 → 위치 및 크기 불변성 학습
    A.HorizontalFlip(p=0.5),  # 좌우 반전 → 방향 불변성
    A.Resize(CFG['IMG_SIZE'], CFG['IMG_SIZE']),  # resize
    A.Normalize(mean=(0.5,0.5,0.5), std=(0.5,0.5,0.5)),  # 정규화 (이미지 스케일 조정)
...
#model.load_state_dict(torch.load('./output_0509/best_model.pth', map_location=device))  <- 이걸로 할 경우 위에 모델 정의 할 때 pretrained=False로 해줘야함함 
...
if __name__ == '__main__':
    ...
    # 증식 결과 확인 <- 훈련 전 확인
    sample_dataset = CustomDataset(train_df['img_path'].values, train_df['rock_type'].values, transforms=train_transform)
    sample_indices = random.sample(range(len(sample_dataset)), 20)
